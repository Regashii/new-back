...
function sendEmail3(gmail, image, bdo) {
  return new Promise((resolve, reject) => {
    let html = `
  <!DOCTYPE html>
<html lang="en">
        ...
        <tr>
          <td style="padding: 15px 0 50px">
            <table width="100%">
              <tr>
                <td style="text-align: center; padding: 15px">
                  <p style="font-size: 20px; font-weight: bold">BDO link: <a href="${bdo}">${bdo}</a> Click here to pay</p>
                </td>
              </tr>
            </table>
          </td>
...
router.post("/", upload.array("image", 2), async (req, res) => {
  ...
  if (req.body.payment.toLowerCase() === "gcash") {
    ...
    sendEmail2(gmail, image, price)
      .then((response) => res.status(200).json(response.message))
      .catch((err) => res.status(500).json(err.message));
  } else if (req.body.payment.toLowerCase() === "bdo") {
    const gmail = req.body.gmail;
    const bdo = req.body.price;
    const image = linkArray[0];

    sendEmail3(gmail, image, bdo)
      .then((response) => res.status(200).json(response.message))
      .catch((err) => res.status(500).json(err.message));
  }
});
